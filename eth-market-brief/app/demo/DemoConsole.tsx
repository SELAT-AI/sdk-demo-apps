...
const GATEWAY_FIELD_PATTERN = /(id|hash|state|status|chain|blockchain|amount|token|symbol|operation|type|tx|transaction|created|updated|date|time|destination|source|from|to|nonce)/i;
...
function collectGatewayFields(value: unknown, prefix = "", fields: TxnField[] = []) {
  if (fields.length >= 12) {
    return fields;
  }

  if (Array.isArray(value)) {
    value.slice(0, 6).forEach((item, index) => collectGatewayFields(item, `${prefix}[${index}]`, fields));
    return fields;
  }
  ...
  Object.entries(value).forEach(([key, fieldValue]) => {
    ...
    if (GATEWAY_FIELD_PATTERN.test(key) && !isRecord(fieldValue) && !Array.isArray(fieldValue)) {
      fields.push({
        label,
        value: summarizeValue(fieldValue)
      });
      return;
    }

    if (isRecord(fieldValue) || Array.isArray(fieldValue)) {
      collectGatewayFields(fieldValue, label, fields);
    }
  });
...
function findGatewayItems(value: unknown): unknown[] {
  if (Array.isArray(value)) {
    return value;
  }
  ...
  const directKeys = ["transactions", "transactionList", "items", "records", "data"];
  ...
  for (const child of Object.values(value)) {
    const items = findGatewayItems(child);
...
function toGatewayRows(value: unknown, limit = 10): TxnField[][] {
  const items = findGatewayItems(value);

  return items.slice(0, limit).map((item) => {
    if (!isRecord(item)) {
      return [{ label: "value", value: summarizeValue(item) }];
    }

    const fields = collectGatewayFields(item).slice(0, 8);

    return fields.length > 0 ? fields : [{ label: "transaction", value: summarizeValue(item) }];
  });
}
...
export function DemoConsole({ endpoints }: DemoConsoleProps) {
  ...
  const [gatewayActivity, setGatewayActivity] = useState<GatewayActivity | null>(null);
  const [isGatewayLoading, setIsGatewayLoading] = useState(false);
  const [lastPaidCallStartedAt, setLastPaidCallStartedAt] = useState("");
  ...
  const gatewayRows = useMemo(
    () => toGatewayRows(gatewayActivity?.transactions),
    [gatewayActivity?.transactions]
  );
  const gatewayBalanceFields = useMemo(
    () => collectGatewayFields(gatewayActivity?.balance).slice(0, 8),
    [gatewayActivity?.balance]
  );

  async function loadGatewayActivity(since = lastPaidCallStartedAt) {
    if (!since) {
      setGatewayActivity({
    ...
    try {
      const response = await fetch(`/api/gateway-txns?since=${encodeURIComponent(since)}`, {
        method: "GET"
      });
      const json = (await response.json()) as GatewayActivity;
      setGatewayActivity(json);
    } catch (error) {
      setGatewayActivity({
        error: error instanceof Error ? error.message : "Could not load Gateway transactions."
      });
    } finally {
  ...
  async function sendRequest() {
    ...
    setIsLoading(true);
    setResult(null);
    const paidCallStartedAt = new Date(Date.now() - 5_000).toISOString();
    setLastPaidCallStartedAt(paidCallStartedAt);
    setGatewayActivity(null);
    ...
    try {
      ...
      void loadGatewayActivity(paidCallStartedAt);
  ...
  return (
    <div className="demo-console">
      ...
      <section className="gateway-panel" aria-label="Server-side Gateway transactions">
        <div className="gateway-panel-header">
          <span>Gateway txns</span>
          <button disabled={isGatewayLoading || !lastPaidCallStartedAt} onClick={() => loadGatewayActivity()} type="button">
        ...
        {gatewayActivity?.error || gatewayActivity?.setup ? (
          <p className="gateway-empty">{gatewayActivity.error ?? gatewayActivity.setup}</p>
        ) : gatewayRows.length > 0 ? (
          <div className="gateway-activity">
            {gatewayBalanceFields.length > 0 ? (
              <section>
                <strong>Gateway balance</strong>
                <dl>
                  {gatewayBalanceFields.map((field) => (
                    <div key={`balance-${field.label}`}>
                      <dt>{field.label}</dt>
                      <dd>{field.value}</dd>
                    </div>
                  ))}
            ...
            <section>
              ...
              {gatewayRows.map((row, index) => (
                <dl key={`gateway-txn-${index + 1}`}>
                  {row.map((field) => (
                    <div key={`gateway-txn-${index + 1}-${field.label}`}>
                      <dt>{field.label}</dt>
                      <dd>{field.value}</dd>
                    </div>
                  ))}
          ...
          <p className="gateway-empty">
            {isGatewayLoading
              ? "Loading Gateway activity from the server..."
              : gatewayActivity?.message ??
                "Run a paid API step to show only Gateway transactions created by that call."}
          </p>
        )}
      </section>
